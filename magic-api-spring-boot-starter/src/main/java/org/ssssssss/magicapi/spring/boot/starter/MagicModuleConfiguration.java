...
import org.ssssssss.magicapi.modules.mqtt.MqttModule;
...
public class MagicModuleConfiguration {
	...
	public MagicModuleConfiguration(MagicAPIProperties properties,
			...
			ObjectProvider<List<ColumnMapperProvider>> columnMapperProvidersProvider, Environment environment) {
	...
	public PageProvider pageProvider() {
		...
		logger.info("未找到分页实现,采用默认分页实现,分页配置:(页码={},页大小={},默认首页={},默认页大小={},最大页大小={})", pageConfig.getPage(),
				pageConfig.getSize(), pageConfig.getDefaultPage(), pageConfig.getDefaultSize(),
				pageConfig.getMaxPageSize());
		return new DefaultPageProvider(pageConfig.getPage(), pageConfig.getSize(), pageConfig.getDefaultPage(),
				pageConfig.getDefaultSize(), pageConfig.getMaxPageSize());
	}
	...
	public SqlCache sqlCache() {
		...
		logger.info("未找到SQL缓存实现，采用默认缓存实现(LRU+TTL)，缓存配置:(容量={},TTL={})", cacheConfig.getCapacity(),
				cacheConfig.getTtl());
	...
	public SQLModule magicSqlModule(MagicDynamicDataSource dynamicDataSource, ResultProvider resultProvider,
			PageProvider pageProvider, SqlCache sqlCache) {
		...
		this.columnMapperProvidersProvider.getIfAvailable(Collections::emptyList).stream()
				.filter(mapperProvider -> !"default".equals(mapperProvider.name())).forEach(columnMapperAdapter::add);
	...
	public RequestModule magicRequestModule(MagicRequestContextHolder magicRequestContextHolder,
		...
		return new RequestModule(magicRequestContextHolder, magicAPIService);
	}
	...
	@Bean
	@ConditionalOnMissingBean({ MagicDynamicRedisClient.class, RedisModule.class })
	public RedisModule magicRedisModule(MagicDynamicRedisClient magicDynamicRedisClient) {
		return new RedisModule(magicDynamicRedisClient);
	}

	@Bean
	@ConditionalOnMissingBean({ MagicDynamicKafkaClient.class, KafkaModule.class })
	public KafkaModule magicKafkaModule(MagicDynamicKafkaClient magicDynamicKafkaClient) {
		return new KafkaModule(magicDynamicKafkaClient);
	}

	@Bean
	@ConditionalOnMissingBean({ MagicDynamicMqttClient.class, MqttModule.class })
	public MqttModule magicMqttModule(MagicDynamicMqttClient magicDynamicMqttClient) {
		return new MqttModule(magicDynamicMqttClient);
	}

	@Bean
	@ConditionalOnMissingBean({ MagicDynamicESClient.class, ESModule.class })
	public ESModule magicESModule(MagicDynamicESClient magicDynamicESClient) {
		return new ESModule(magicDynamicESClient);
	}
	@Bean
	@ConditionalOnMissingBean({ MagicDynamicHbaseClient.class, HbaseModule.class })
	public HbaseModule magicHbaseModule(MagicDynamicHbaseClient magicDynamicHbaseClient) {
		return new HbaseModule(magicDynamicHbaseClient);
	}

	@Bean
	@ConditionalOnMissingBean({ MagicDynamicTcpClient.class, TcpModule.class })
	public TcpModule magicTcpModule(MagicDynamicTcpClient magicDynamicTcpClient) {
