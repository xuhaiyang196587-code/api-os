...
import org.ssssssss.magicapi.utils.APIUtils;
...
//		System.out.println("\t可通过配置关闭输出: \tmagic-api.show-url=false");
		System.out.println("********************************************当前服务相关地址********************************************");
		APIUtils apiUtils = new APIUtils(apiService, service);
		List<Map<String,Object>> apis = apiUtils.getApis();
		for (Map<String, Object> api : apis) {
			if(api.get("path").equals("dynamic")) {
				try {
					apiUtils.exeApiLeaf(api);
				} catch (Exception e) {
					e.printStackTrace();
				} 
			}
